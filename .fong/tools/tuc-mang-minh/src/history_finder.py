        ...
        if entry.is_dir():
            ...
            if metadata_file.is_file():
                try:
                    ...
                    filename_only = Path(file_path_to_find).name
                    
                    # Try exact match first, then fallback to filename match
                    exact_match = resource == f"file://{file_path_to_find_abs}"
                    filename_match = filename_only in resource and resource.startswith("file://")
                    
                    if not (exact_match or filename_match):
                        continue
