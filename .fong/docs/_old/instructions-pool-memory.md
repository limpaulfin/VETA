...
Quy tắc này định nghĩa cách tạo, lưu trữ và tìm kiếm thông tin trong thư mục pool-memory (`$HOME/Dropbox/pool-memory/`) - một kho lưu trữ tập trung cho các file JSON có cấu trúc chuẩn và machine-readable.
...
grep -r -i -E "keyword1|keyword2|keyword3" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm kiếm theo task name
grep -r -i "task_name.*similar_task" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm kiếm theo tags
grep -r -i '"tags".*"relevant_tag"' "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
```bash
# Sử dụng MCP fong-linux-command-executor
mv /path/to/source/file.json "$HOME/Dropbox/pool-memory/"
```
...
find "$HOME/Dropbox/pool-memory/" -name "*UUID_HERE*" -type f

# Tìm trong nội dung file có UUID
grep -r "UUID_HERE" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
grep -r -i -E "task_name.*(chrome_profile|browser_profile|user_profile)" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file có chứa từ khóa task với precise terminology
grep -r -i -E "(chrome.*profile|browser.*profile|user.*profile)" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm kiếm mở rộng với từ khóa liên quan
grep -r -i -E "(profile.*discovery|profile.*analysis|profile.*inventory)" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
grep -r -i -E '"tags".*"(system_analysis|system_inventory|configuration_audit)"' "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file có nhiều tags kết hợp
grep -r -i -E '"tags".*"(chrome_profiles|browser_profiles)".*"(discovery|analysis|inventory)"' "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm kiếm với tags tiếng Anh chính xác
grep -r -i -E '"tags".*"(profile_mapping|user_accounts|browser_configuration)"' "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
grep -r "2025-07-27" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file theo unix timestamp
grep -r "1753619500" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
grep -r "ubuntu-lenovo-t14" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file theo OS platform
grep -r "Ubuntu Linux" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
grep -r '"process_status".*"completed"' "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file đang thực hiện
grep -r '"process_status".*"in_progress"' "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
grep -r -A 3 -B 3 -E "(system_analysis|profile_discovery|configuration_audit)" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file có nhiều từ khóa tiếng Anh chính xác
grep -r -l -E "(chrome.*profile|browser.*profile)" "$HOME/Dropbox/pool-memory/" --include="*.json" | xargs grep -l -E "(discovery|analysis|inventory)"

# Tìm kiếm với từ khóa liên quan và context
grep -r -A 5 -B 5 -E "(user_accounts|profile_mapping|browser_configuration)" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
find "$HOME/Dropbox/pool-memory/" -name "*.json" -type f

# Đếm số file
find "$HOME/Dropbox/pool-memory/" -name "*.json" -type f | wc -l

# Tìm file theo pattern tên
find "$HOME/Dropbox/pool-memory/" -name "*chrome*" -type f
```
...
mv /tmp/process-log.json "$HOME/Dropbox/pool-memory/system-analysis-$UUID.json"
```
...
grep -r -i -E "(chrome|browser|profile)" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file tạo hôm nay
grep -r "$(date '+%Y-%m-%d')" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm file có tag system_analysis với precise English
grep -r -E '"tags".*"(system_analysis|system_inventory|configuration_audit)"' "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm kiếm với nhiều từ khóa liên quan
grep -r -i -E "(profile_discovery|browser_configuration|user_accounts)" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
...
ls -la "$HOME/Dropbox/pool-memory/"

# Kiểm tra quyền truy cập
ls -la "$HOME/Dropbox/pool-memory/"*.json
```
...
# Sử dụng MCP fong-linux-command-executor
# Validate JSON
cat "$HOME/Dropbox/pool-memory/"file.json | jq '.'

# Kiểm tra syntax
python3 -m json.tool "$HOME/Dropbox/pool-memory/"file.json
```
...
wc -c "$HOME/Dropbox/pool-memory/"file.json

# Đếm words (rough estimate: ~1.3 words per token)
wc -w "$HOME/Dropbox/pool-memory/"file.json

# Kiểm tra với tiktoken (nếu có)
python3 -c "import tiktoken; enc = tiktoken.get_encoding('cl100k_base'); print('Tokens:', len(enc.encode(open('"$HOME/Dropbox/pool-memory/"file.json').read())))" 2>/dev/null || echo "tiktoken not available"

# Ước tính token từ characters (conservative)
echo "Estimated tokens: $(($(wc -c < "$HOME/Dropbox/pool-memory/"file.json) / 4))"
```
...
# Sử dụng MCP fong-linux-command-executor
# Mở rộng từ khóa
grep -r -i "chrome\|browser\|profile" "$HOME/Dropbox/pool-memory/" --include="*.json"

# Tìm với context
grep -r -A 5 -B 5 "keyword" "$HOME/Dropbox/pool-memory/" --include="*.json"
```
