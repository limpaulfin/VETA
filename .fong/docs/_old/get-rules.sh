...
# Mod by Fong on 2025-07-24--11-51-AM

# === Đồng bộ quy tắc từ Project về Project (Local Copy) ===
# Script này đồng bộ các file quy tắc (.mdc) và các công cụ mini từ thư mục .cursor/rules/
# của dự án về chính nó (local copy). Được sử dụng khi cần refresh rules.
# Cảnh báo: Lệnh này sẽ ghi đè lên các file trong thư mục đích nếu file
# ở thư mục nguồn mới hơn.

# Định nghĩa các biến
SOURCE_DIR="/home/fong/Dropbox/Projects/boiler-plate-cursor-project-with-init-prompt/.cursor/rules/"
DEST_DIR="./.cursor/rules/"

# Kiểm tra xem thư mục nguồn có tồn tại không
if [ ! -d "$SOURCE_DIR" ]; then
  echo "Loi: Thu muc nguon khong ton tai: $SOURCE_DIR"
  exit 1
fi

# Thông báo bắt đầu
echo "Bat dau dong bo tat ca rules va tools tu project: $SOURCE_DIR"
...
# Thông báo hoàn tất
echo "Dong bo toan bo rules va tools tu project ve du an hoan tat." 