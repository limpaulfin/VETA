...
## ⚠️ Production Environment - lms.deutschfuns.com ONLY
...
## ⚠️ Prerequisite: Navigate to Project Root
...
- Returns hierarchical structure: courses → lessons → topics → quizzes → questions
...
### ✅ PRODUCTION-READY: curlx.sh with lms.deutschfuns.com

```bash
# 🎯 PRIMARY: Get full template for all courses
$DE_PROJECT_ROOT/.fong/script-sh/curlx.sh 'curl -s -k "https://lms.deutschfuns.com/?fong-debug-test=initialize-learning-progress-template"'

# 🎯 ALTERNATIVE: Get template with caching enabled
$DE_PROJECT_ROOT/.fong/script-sh/curlx.sh 'curl -s -k "https://lms.deutschfuns.com/?fong-debug-test=initialize-learning-progress-template&use_cache=true"'

# 🎯 TARGETED: Get template for specific courses only
$DE_PROJECT_ROOT/.fong/script-sh/curlx.sh 'curl -s -k "https://lms.deutschfuns.com/?fong-debug-test=initialize-learning-progress-template&course_ids=7702,10456"'
```
...
- ✅ **Auto SSH proxy** to `fong@192.168.122.1`
- ✅ **Auto save** to `.fong/_tmp/curl_[timestamp].txt` 
- ✅ **Clean JSON** output with `-s -k` flags
- ✅ **Preview** first 10 lines + file info
- ✅ **Production safe** with whitelisted endpoint
- ✅ **Large output handling** (3.5MB+ templates supported)
...
- **Production blueprint**: `$DE_PROJECT_ROOT/.fong/instructions/instructionsX-endpointx-staging-production-allow.md`
