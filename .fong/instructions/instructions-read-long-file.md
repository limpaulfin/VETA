# Instructions: Smart Long File Reading

*Modified by Fong on 2025-06-15--19-18-PM*

## ⚠️ QUAN TRỌNG: Sử dụng MCP Tools

**Khi tìm kiếm và đọc file, LUÔN ưu tiên sử dụng MCP tools thay vì internal tools:**

### MCP Tools Available:
- **`mcp_fong-linux-command-executor_runLinuxCommand`**: Thay thế `run_terminal_cmd` cho Linux commands
- **`mcp_fong-long-file-reader_readFileWithCommands`**: Thay thế `read_file` cho file dài
- **`mcp_fong-php-file-reader_readPhpFile`**: Đọc PHP files với parsing đúng
- **`mcp_fong-js-file-reader_readJsFile`**: Đọc JS files với dependency analysis

### Ví dụ MCP Usage:
```bash
# OLD: run_terminal_cmd
# NEW: mcp_fong-linux-command-executor_runLinuxCommand

# OLD: read_file
# NEW: mcp_fong-long-file-reader_readFileWithCommands

# OLD: ls -la
# NEW: tree -p (luôn dùng tree thay vì ls)
```

## Mục đích

Quy tắc này hướng dẫn cách đọc file dài một cách thông minh và hiệu quả, đặc biệt khi:
- AI hit giới hạn 200 dòng của `mcp_fong-long-file-reader_readFileWithCommands` tool
- File có cấu trúc phức tạp hoặc nhiều thông tin không cần thiết
- Cần tìm kiếm thông tin cụ thể trong file lớn
